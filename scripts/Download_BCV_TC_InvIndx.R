...
scrape_bcv_fx_page <- function(page_number) {
  ...
  clean_table <- raw_table %>%
    select(fecha = 1, tasa = 2) %>%  # Keep only date + exchange rate columns
    mutate(
      fecha = dmy(fecha),
      tasa = str_replace(tasa, ",", ".") %>% as.numeric()
    )
  
  return(clean_table)
}

# Identify max number of pages from BCV webside
max_page <- 27

# Scrape all pages with progress messages
